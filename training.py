...
def generate_datasets(FILENAME):
    ...
    return dataloader_tr, dataset, dataframe

##### Vocab work #####
def genereate_vocab(dataframe, dataset, save=False):
    ...
    return english, PAD_IDX, VOCAB_LEN

##### Prepare Model, Optimizer and Criterion #####
print('Creating Models')
def generate_models(VOCAB_LEN, PAD_IDX, EMBEDDING_SIZE, ATTENTION_HEADS, DROPOUT, USE_DOM, LR):
    ...
    return multitask, adam, scheduler, valence_L, arousal_L, dominance_L, quad_L

# Training Loop

def train_model(EPOCHS, dataloader_tr, DEVICE, PRINT_STEP, SAVE_STEP, multitask, adam, scheduler, valence_L, arousal_L, dominance_L, quad_L):
...
dataloader_tr, dataset, dataframe = generate_datasets(FILENAME)
english, PAD_IDX, VOCAB_LEN = genereate_vocab(dataframe, dataset, True)
lrs = [5e-3, 1e-3, 5e-4, 1e-4]
cross_val_losses = {l : [] for l in lrs}
for lr in lrs:
    print(f'Using LR {lr}')
    multitask, adam, scheduler, valence_L, arousal_L, dominance_L, quad_L = generate_models(VOCAB_LEN, PAD_IDX, EMBEDDING_SIZE, ATTENTION_HEADS, DROPOUT, USE_DOM, lr)
    cross_val_losses[lr] = train_model(2, dataloader_tr, DEVICE, PRINT_STEP, 5, multitask, adam, scheduler, valence_L, arousal_L, dominance_L, quad_L)
